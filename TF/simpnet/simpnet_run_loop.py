...
parsing. Code for defining the SimpNet layers can be found in simpnet_model.py.
"""
...
################################################################################
def learning_rate_with_decay(boundary_epochs, initial_lr, gamma, batch_size, num_images):
  """Get a learning rate that decays step-wise as training progresses.

  Args:
    boundary_epochs: list of ints representing the epochs at which we
      decay the learning rate. (steps at which the learning rate is
      decayed!)
    initial_lr: initial learning rate
    gamma: a float number which is multiplied by the learning rate, 
      to give the new learning rate  
    batch_size: training set batch size, used to calculate the epochs 
    num_images: number of images in training set.
      used to calculate epochs.   
  ...
  """
  initial_learning_rate = initial_lr #0.1 * batch_size / batch_denom
  batches_per_epoch = num_images / batch_size

  # Multiply the learning rate by 0.1 at 100, 190, 306, 390, 440 and 540 epochs.
  boundaries = [int(batches_per_epoch * epoch) for epoch in boundary_epochs]
  vals=[]
  vals.append(initial_lr)
  for i in range(len(boundaries)):
    initial_lr = initial_lr * gamma
    vals.append(initial_lr)
  # print('steps: ',boundaries)
  # print('learning rates: ',vals)  
...
def simpnet_model_fn(features, labels, mode, model_class,
  ...
  model = model_class(data_format, dtype=dtype)
...
class SimpNetArgParser(argparse.ArgumentParser):
  ...
  def parse_args(self, args=None, namespace=None):
    args = super(SimpNetArgParser, self).parse_args(
