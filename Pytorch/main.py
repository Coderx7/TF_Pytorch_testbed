...
parser.add_argument('--arch', metavar='ARCH', default='resnet18', choices=model_names, help='model architecture: ' + ' | '.join(model_names) + ' (default: simpnet)')
...
def train(train_loader, model, criterion, optimizer, epoch, log):
  ...
  for i, (input, target) in enumerate(train_loader):
    ...
    losses.update(loss.item(), input.size(0)) #(loss.data[0], input.size(0))
    top1.update(prec1.item(), input.size(0)) # (prec1[0], input.size(0))
    top5.update(prec5.item(), input.size(0)) # (prec5[0], input.size(0))
  ...
  with torch.no_grad(): # for versions >=0.4
    for i, (input, target) in enumerate(val_loader):
      if args.use_cuda:
        target = target.cuda(async=True)
        input = input.cuda()
      input_var = torch.autograd.Variable(input) #, volatile=True) for version <0.4 uncomment
...
def save_checkpoint(state, is_best, save_path, filename):
  ...
  bestname = os.path.join(save_path, 'best_{0}'.format(filename))
